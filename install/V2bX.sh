...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/svip7788/V2bX/master/install/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/svip7788/V2bX/master/install/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/svip7788/V2bX/master/install/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/svip7788/V2bX ---
  ${green}0.${plain} 修改配置
————————————————
